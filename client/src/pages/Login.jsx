  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', fontFamily:"'DM Sans','Segoe UI',sans-serif" }}>
      ...
      <div style={{ flex:1, background:'linear-gradient(160deg,#0f0a1e,#1a0f3c)', display:'flex', flexDirection:'column', justifyContent:'center', padding:'60px', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ position:'absolute', width:400, height:400, borderRadius:'50%', background:'rgba(109,40,217,0.15)', filter:'blur(80px)', top:'-20%', right:'-10%' }} />
        ...
        <div style={{ position:'relative', zIndex:1 }}>
          ...
          <h2 style={{ color:'#fff', fontSize:36, fontWeight:800, letterSpacing:'-1px', lineHeight:1.2, marginBottom:16, fontFamily:"'DM Serif Display',serif" }}>
            Navigate India<br />smarter today
          </h2>
          <p style={{ color:'rgba(255,255,255,0.55)', fontSize:16, lineHeight:1.7, marginBottom:48, maxWidth:340 }}>
            AI-powered traffic prediction, live GPS navigation and fleet management — all in one app.
          </p>

          {/* Feature pills */}
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            {[
              { icon:'🗺️', text:'Compare 3 routes simultaneously' },
              { icon:'🤖', text:'AI congestion prediction with XGBoost' },
              { icon:'🚛', text:'Fleet management for businesses' },
              { icon:'📍', text:'Live GPS with voice navigation' },
            ].map((f,i) => (
              <div key={i} style={{ display:'flex', alignItems:'center', gap:12, animation:'fadeUp 0.6s ease forwards', animationDelay:`${i*0.1+0.2}s`, opacity:0 }}>
                <div style={{ width:32, height:32, borderRadius:8, background:'rgba(109,40,217,0.3)', border:'1px solid rgba(109,40,217,0.4)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:16, flexShrink:0 }}>{f.icon}</div>
      ...
      {/* Right — login form */}
      <div style={{ width:'100%', maxWidth:480, display:'flex', flexDirection:'column', justifyContent:'center', padding:'48px 48px', background:'#fff', overflowY:'auto' }}>
        <div style={{ animation:'fadeUp 0.6s ease forwards' }}>
          <div style={{ marginBottom:32 }}>
            <h1 style={{ fontSize:28, fontWeight:800, color:'#111', letterSpacing:'-0.5px', marginBottom:8 }}>Welcome back</h1>
            <p style={{ fontSize:15, color:'#6b7280' }}>
              Don't have an account?{' '}
              <Link to="/signup" style={{ color:'#6d28d9', fontWeight:600, textDecoration:'none' }}>Sign up free</Link>
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display:'flex', flexDirection:'column', gap:16 }}>
            ...
            <div>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:6 }}>
                ...
                <a href="#" style={{ fontSize:13, color:'#6d28d9', textDecoration:'none' }}>Forgot password?</a>
              </div>
              <input
          ...
          <p style={{ marginTop:32, fontSize:13, color:'#9ca3af', textAlign:'center', lineHeight:1.6 }}>
            By signing in you agree to our{' '}
            <a href="#" style={{ color:'#6d28d9', textDecoration:'none' }}>Terms of Service</a>
            {' '}and{' '}
