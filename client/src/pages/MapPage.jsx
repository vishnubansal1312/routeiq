    ...
    <div className="flex h-[calc(100vh-64px)]">
      ...
      <div className="w-80 bg-dark-800 border-r border-dark-700 flex flex-col overflow-y-auto">
        <div className="p-4 space-y-4">
          <h2 className="text-sm font-bold text-slate-300 uppercase tracking-wider">Plan Your Route</h2>

          <SearchBox label="Origin" placeholder="e.g. Connaught Place, Delhi"
            value={origin.label}
          ...
          <div>
            ...
            <div className="flex gap-2">
              <button onClick={() => setCompareMode(true)} className={`flex-1 py-2 text-xs font-semibold rounded-lg border transition-all ${compareMode ? 'bg-primary-500 text-white border-primary-500' : 'bg-dark-700 text-slate-400 border-dark-600 hover:bg-dark-600'}`}>
                Compare all 3
              </button>
              <button onClick={() => setCompareMode(false)} className={`flex-1 py-2 text-xs font-semibold rounded-lg border transition-all ${!compareMode ? 'bg-primary-500 text-white border-primary-500' : 'bg-dark-700 text-slate-400 border-dark-600 hover:bg-dark-600'}`}>
                Single route
              </button>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <TrafficIncidents enabled={showIncidents} onToggle={() => setShowIncidents(s => !s)} incidentCount={incidentCount} onCountChange={setIncidentCount} />
            <AccidentHotspots enabled={showHotspots} onToggle={() => setShowHotspots(s => !s)} count={hotspotCount} />
          </div>
          ...
          <div>
            ...
            {!tracking ? (
              <button onClick={startTracking} className="w-full py-2.5 text-xs font-semibold rounded-xl border flex items-center justify-center gap-2 bg-blue-500/10 text-blue-400 border-blue-500/30 hover:bg-blue-500/20 transition-all">
                <span className="w-2 h-2 bg-blue-400 rounded-full" />Start Live Navigation
              </button>
            ) : (
              <div className="space-y-2">
                <div className="flex items-center justify-between py-2 px-3 bg-blue-500/10 border border-blue-500/30 rounded-xl">
                  ...
                  {userLocation && <span className="text-xs text-slate-400 font-mono">{userLocation.lat.toFixed(4)}, {userLocation.lon.toFixed(4)}</span>}
                </div>
                <div className="flex gap-2">
                  <button onClick={() => setFollowUser(f => !f)} className={`flex-1 py-2 text-xs rounded-lg border transition-all ${followUser ? 'bg-primary-500/20 text-primary-400 border-primary-500/40' : 'bg-dark-700 text-slate-400 border-dark-600'}`}>
                    {followUser ? 'Following' : 'Follow me'}
                  </button>
                  <button onClick={stopTracking} className="flex-1 py-2 text-xs rounded-lg border bg-red-500/10 text-red-400 border-red-500/30 hover:bg-red-500/20 transition-all">
                    Stop GPS
                  </button>
                </div>
              </div>
            )}
            {locationError && <div className="mt-2 text-xs text-red-400 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">{locationError}</div>}
          </div>

          {error && <div className="bg-red-500/10 border border-red-500/30 text-red-400 rounded-xl px-3 py-2 text-xs">{error}</div>}
          ...
          {tripSaved && <div className="text-center text-xs text-green-400 bg-green-400/10 border border-green-400/20 rounded-lg py-2">Trip saved to history</div>}
        </div>
            ...
            {weather && (
              <div className="bg-dark-700 rounded-xl p-3">
                ...
                {weather.alerts?.map((alert,i) => (
                  <div key={i} className={`mt-2 text-xs px-3 py-2 rounded-lg ${alert.type==='danger' ? 'bg-red-500/10 text-red-400 border border-red-500/20' : alert.type==='warning' ? 'bg-orange-500/10 text-orange-400 border border-orange-500/20' : 'bg-blue-500/10 text-blue-400 border border-blue-500/20'}`}>
                    {alert.message}
                  </div>
                ))}
              </div>
      ...
      <div className="flex-1 relative">
        <MapContainer center={[20.5937, 78.9629]} zoom={5} style={{ height:'100%', width:'100%' }}>
          ...
          {compareMode
            ? allRoutes.map(route => (
                <Polyline key={route.key} positions={route.points} color={ROUTE_COLORS[route.key]}
                  weight={activeRoute===route.key ? 6 : 3} opacity={activeRoute===route.key ? 0.95 : 0.45}
                  eventHandlers={{ click: () => setActiveRoute(route.key) }} />
              ))
            : activeRouteData && <Polyline positions={activeRouteData.points} color={ROUTE_COLORS[activeRoute]} weight={5} opacity={0.9} />
          }
          ...
          {userLocation && (
            <>
              ...
              <Circle center={[userLocation.lat, userLocation.lon]} radius={80} pathOptions={{ color:'#3b82f6', fillColor:'#3b82f6', fillOpacity:0.1, weight:1 }} />
        ...
        {compareMode && allRoutes.length > 0 && (
          <div className="absolute top-4 right-4 bg-dark-800/90 backdrop-blur-sm border border-dark-600 rounded-xl px-4 py-3 space-y-2">
            ...
            {[{key:'fastest',color:'#0ea5e9',label:'Fastest'},{key:'shortest',color:'#a855f7',label:'Shortest'},{key:'eco',color:'#22c55e',label:'Eco'}].map(r => (
              <div key={r.key} onClick={() => setActiveRoute(r.key)} className="flex items-center gap-2 cursor-pointer hover:opacity-80">
                <div className="w-6 h-1.5 rounded-full" style={{ backgroundColor:r.color, opacity:activeRoute===r.key ? 1 : 0.45 }} />
                <span className={`text-xs ${activeRoute===r.key ? 'text-white font-semibold' : 'text-slate-400'}`}>{r.label}</span>
        ...
        {tracking && !followUser && (
          <button onClick={() => setFollowUser(true)} className="absolute bottom-24 right-6 bg-blue-500 hover:bg-blue-600 text-white text-xs font-semibold px-4 py-2.5 rounded-xl shadow-lg flex items-center gap-2">
            <span className="w-2 h-2 bg-white rounded-full animate-pulse" />Re-center on me
          </button>
        )}
        ...
        <ChatAssistant
          routeContext={{
            ...
            weather: weather, activeRoute: activeRoute,
          }}
        />
      </div>
    </div>
