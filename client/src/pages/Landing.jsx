    ...
    <div style={{ fontFamily: "'DM Sans', 'Segoe UI', sans-serif", background: '#fff', color: '#111', overflowX: 'hidden' }}>
      ...
      <style>{`
        ...
        .route-line {
          ...
          animation: dash 2s cubic-bezier(0.4,0,0.2,1) 0.8s forwards;
        }
        ...
        <div style={{ maxWidth:1200, margin:'0 auto', height:64, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:32 }}>
            {[['Features','#features'],['How it works','#how'],['Fleet','#fleet'],['Pricing','#pricing']].map(([label,href]) => (
              <a key={label} href={href} className="nav-link" style={{ display:'none' }} id="desktop-nav">{label}</a>
            ))}
      ...
      <section style={{
        ...
        <div className="hero-grid" style={{ maxWidth:1200, width:'100%', display:'grid', gridTemplateColumns:'1fr 1fr', gap:60, alignItems:'center', position:'relative', zIndex:1 }}>
          ...
          <div>
            ...
            <div className="fade-up fade-up-4" style={{ display:'flex', gap:12, flexWrap:'wrap' }}>
              <Link to="/signup">
                <button className="btn-primary" style={{ fontSize:16, padding:'16px 36px' }}>Start for free →</button>
              </Link>
              <a href="#features">
          ...
          {/* Right — map visual */}
          <div className="hero-visual" style={{ position:'relative', display:'flex', alignItems:'center', justifyContent:'center', minHeight:480 }}>
            <div style={{ width:'100%', maxWidth:440, background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:24, padding:24, backdropFilter:'blur(20px)' }} className="float-card">
              ...
              {/* Mini map SVG */}
              <svg width="100%" viewBox="0 0 380 260" style={{ borderRadius:16, overflow:'hidden', display:'block' }}>
                <defs>
                  {/* Glow filters for each route color */}
                ...
                {/* Background road shadows */}
                <path d="M 30 210 Q 100 185 190 135 Q 280 88 355 65" fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth="8" strokeLinecap="round" />

                {/* ── ROUTE LINES — thick + glowing ── */}

                {/* Fastest — bright cyan blue */}
                <path
                  d="M 50 200 Q 110 172 190 130 Q 270 88 345 68"
                  fill="none"
                  stroke="#38bdf8"
                  strokeWidth="5"
                  strokeLinecap="round"
                ...
                {/* Shortest — bright purple */}
                <path
                  d="M 50 200 Q 90 162 150 118 Q 210 74 345 68"
                  fill="none"
                  stroke="#c084fc"
                  strokeWidth="4"
                  strokeLinecap="round"
                  ...
                  style={{ animation:'dash 2s cubic-bezier(0.4,0,0.2,1) 1.2s forwards' }}
                />

                {/* Eco — bright green */}
                <path
                  d="M 50 200 Q 122 178 202 140 Q 282 104 345 68"
                  fill="none"
                  stroke="#4ade80"
                  strokeWidth="4"
                  strokeLinecap="round"
                  ...
                  style={{ animation:'dash 2s cubic-bezier(0.4,0,0.2,1) 1.6s forwards' }}
                />

                {/* Origin dot — green */}
                <circle cx="50" cy="200" r="10" fill="#22c55e" />
                <circle cx="50" cy="200" r="18" fill="rgba(34,197,94,0.25)" />
                <circle cx="50" cy="200" r="26" fill="rgba(34,197,94,0.1)" />

                {/* Destination dot — red */}
                <circle cx="345" cy="68" r="10" fill="#ef4444" />
                <circle cx="345" cy="68" r="18" fill="rgba(239,68,68,0.25)" />
                <circle cx="345" cy="68" r="26" fill="rgba(239,68,68,0.1)" />

                {/* GPS live dot — blue */}
                <circle cx="195" cy="130" r="7" fill="#3b82f6" />
                <circle cx="195" cy="130" r="14" fill="rgba(59,130,246,0.25)" style={{ animation:'pulse-ring 1.5s ease-out infinite' }} />

                {/* Labels */}
                <rect x="58" y="188" width="74" height="18" rx="4" fill="rgba(0,0,0,0.5)" />
                <text x="95" y="200" fill="rgba(255,255,255,0.85)" fontSize="11" fontFamily="sans-serif" textAnchor="middle" fontWeight="600">New Delhi</text>

                <rect x="270" y="56" width="54" height="18" rx="4" fill="rgba(0,0,0,0.5)" />
                <text x="297" y="68" fill="rgba(255,255,255,0.85)" fontSize="11" fontFamily="sans-serif" textAnchor="middle" fontWeight="600">Mumbai</text>
              </svg>

              {/* Route legend below map */}
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginTop:16 }}>
                {[
                  { label:'Fastest',  color:'#38bdf8', bg:'rgba(56,189,248,0.1)',  border:'rgba(56,189,248,0.3)',  time:'18h 20m', dist:'1,420 km' },
                  { label:'Shortest', color:'#c084fc', bg:'rgba(192,132,252,0.1)', border:'rgba(192,132,252,0.3)', time:'19h 05m', dist:'1,380 km' },
                  { label:'Eco',      color:'#4ade80', bg:'rgba(74,222,128,0.1)',  border:'rgba(74,222,128,0.3)',  time:'20h 10m', dist:'1,395 km' },
                ].map(r => (
                  <div key={r.label} style={{ background:r.bg, borderRadius:12, padding:'10px', border:`1px solid ${r.border}` }}>
                    <div style={{ fontSize:10, color:r.color, fontWeight:700, marginBottom:4 }}>{r.label}</div>
                    <div style={{ fontSize:14, color:'#fff', fontWeight:800 }}>{r.time}</div>
                    <div style={{ fontSize:10, color:'rgba(255,255,255,0.45)', marginTop:2 }}>{r.dist}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Floating badges */}
            <div style={{ position:'absolute', top:20, left:-20, background:'rgba(34,197,94,0.15)', border:'1px solid rgba(34,197,94,0.4)', borderRadius:12, padding:'10px 14px', backdropFilter:'blur(20px)' }}>
              <div style={{ fontSize:10, color:'#4ade80', fontWeight:700 }}>CONGESTION</div>
              <div style={{ fontSize:20, color:'#fff', fontWeight:800 }}>Low</div>
              <div style={{ fontSize:10, color:'rgba(255,255,255,0.4)' }}>Score 2.3/10</div>
            </div>

            <div style={{ position:'absolute', bottom:40, right:-20, background:'rgba(56,189,248,0.15)', border:'1px solid rgba(56,189,248,0.4)', borderRadius:12, padding:'10px 14px', backdropFilter:'blur(20px)' }}>
              <div style={{ fontSize:10, color:'#38bdf8', fontWeight:700 }}>WEATHER</div>
              <div style={{ fontSize:20, color:'#fff', fontWeight:800 }}>26°C</div>
        ...
        <div className="stats-grid" style={{ maxWidth:1200, margin:'0 auto', display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:32 }}>
          {STATS.map((s,i) => (
            <div key={i} style={{ textAlign:'center' }}>
              <div style={{ fontSize:42, fontWeight:800, color:'#fff', letterSpacing:'-1px', lineHeight:1 }}>
      ...
      <section id="fleet" style={{ padding:'100px 24px', background:'linear-gradient(135deg,#0f0a1e 0%,#1a0f3c 100%)', overflow:'hidden' }}>
        <div className="fleet-grid" style={{ maxWidth:1200, margin:'0 auto', display:'grid', gridTemplateColumns:'1fr 1fr', gap:80, alignItems:'center' }}>
          <div>
            ...
            <Link to="/fleet">
              <button className="btn-primary" style={{ background:'#a78bfa', fontSize:15 }}>Try fleet demo →</button>
            </Link>
          </div>
      ...
      <section id="pricing" style={{ padding:'100px 24px', background:'#fafafa' }}>
        <div style={{ maxWidth:1200, margin:'0 auto' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))', gap:20, maxWidth:960, margin:'0 auto' }}>
            {[
              { name:'Free',     price:'₹0',    period:'forever', features:['5 route searches/day','Basic congestion prediction','Weather at destination','7-day trip history'],                                                                                 cta:'Get started', highlight:false },
              { name:'Pro',      price:'₹499',  period:'/month',  features:['Unlimited searches','AI Route Assistant','All 3 routes + comparison','Best departure time AI','Accident hotspot map','Carbon footprint score'],                                     cta:'Start Pro',   highlight:true  },
              { name:'Business', price:'₹1,499',period:'/month',  features:['Everything in Pro','Fleet management','50 vehicles tracking','5 team accounts','Priority support','API access'],                                                                    cta:'Contact us',  highlight:false },
            ].map((plan,i) => (
              <div key={i} style={{ background:plan.highlight?'#6d28d9':'#fff', borderRadius:24, padding:32, border:plan.highlight?'none':'1px solid rgba(0,0,0,0.08)', position:'relative' }}>
                {plan.highlight && (
                  <div style={{ position:'absolute', top:-14, left:'50%', transform:'translateX(-50%)', background:'#a78bfa', color:'#fff', fontSize:11, fontWeight:700, padding:'5px 18px', borderRadius:100, whiteSpace:'nowrap' }}>
                    Most popular
                  </div>
                )}
                ...
                <Link to={i===2?'/pricing':'/signup'}>
                  <button
                    style={{ width:'100%', padding:'14px', borderRadius:12, fontSize:15, fontWeight:600, cursor:'pointer', border:plan.highlight?'none':'1.5px solid #6d28d9', background:plan.highlight?'rgba(255,255,255,0.15)':'transparent', color:plan.highlight?'#fff':'#6d28d9', transition:'all 0.2s' }}
      ...
      <section style={{ padding:'80px 24px', background:'#6d28d9' }}>
        <div style={{ maxWidth:700, margin:'0 auto', textAlign:'center' }}>
          ...
          <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap' }}>
            <Link to="/signup">
              <button
                style={{ background:'#fff', color:'#6d28d9', border:'none', padding:'16px 36px', borderRadius:12, fontSize:16, fontWeight:700, cursor:'pointer', transition:'all 0.2s' }}
            ...
            <Link to="/map">
              <button style={{ background:'transparent', color:'#fff', border:'1.5px solid rgba(255,255,255,0.4)', padding:'15px 28px', borderRadius:12, fontSize:16, fontWeight:600, cursor:'pointer' }}>
                Try the map
              </button>
            </Link>
          </div>
      ...
      <footer style={{ background:'#0f0a1e', padding:'48px 24px 32px' }}>
        <div style={{ maxWidth:1200, margin:'0 auto' }}>
          ...
          <div style={{ borderTop:'1px solid rgba(255,255,255,0.08)', paddingTop:24, display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:12 }}>
            <span style={{ fontSize:13, color:'rgba(255,255,255,0.3)' }}>© 2026 RouteIQ. Built with React, Node.js and Python.</span>
