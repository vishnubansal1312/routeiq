...
function ProgressBar({ covered, total }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:6 }}>
      <div style={{ flex:1, height:5, background:'#f0f0f0', borderRadius:3 }}>
        <div style={{ width:`${pct}%`, height:5, background:'#6d28d9', borderRadius:3 }} />
      </div>
      <span style={{ fontSize:10, color:'#9ca3af', minWidth:36 }}>{pct}%</span>
    </div>
  )
}
...
function MapFly({ vehicles, selected }) {
  ...
  useEffect(() => {
    if (selected) {
      ...
      map.fitBounds(L.latLngBounds(vehicles.map(v => [v.lat, v.lon])), { padding:[50,50] })
    }
  }, [selected, vehicles])
  return null
}
...
export default function Fleet() {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'calc(100vh - 56px)', fontFamily:"'DM Sans','Segoe UI',sans-serif" }}>
      ...
      {/* Demo banner */}
      <div style={{ background:'#faf5ff', borderBottom:'1px solid #e9d5ff', padding:'10px 16px', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0, gap:8, flexWrap:'wrap' }}>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          <span style={{ background:'#6d28d9', color:'#fff', fontSize:11, fontWeight:700, padding:'3px 10px', borderRadius:20, whiteSpace:'nowrap' }}>DEMO</span>
          <span style={{ color:'#6b7280', fontSize:13 }}>Simulated fleet — upgrade for real GPS tracking</span>
        </div>
        <button onClick={() => setShowUpgrade(true)}
          style={{ background:'#6d28d9', color:'#fff', border:'none', borderRadius:8, padding:'7px 14px', fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'inherit', whiteSpace:'nowrap' }}>
          Upgrade →
        </button>
      </div>
      ...
      {isMobile && (
        <div style={{ display:'flex', borderBottom:'1px solid #f0f0f0', flexShrink:0 }}>
          <button onClick={() => setShowMap(false)}
            style={{ flex:1, padding:'12px', border:'none', background:!showMap?'#f5f3ff':'#fff', color:!showMap?'#6d28d9':'#6b7280', fontSize:13, fontWeight:!showMap?700:500, cursor:'pointer', fontFamily:'inherit', borderBottom:!showMap?'2px solid #6d28d9':'none' }}>
            📋 Vehicles
          </button>
      ...
      <div style={{ display:'flex', flex:1, overflow:'hidden', flexDirection:'row' }}>
        ...
        {(!isMobile || !showMap) && (
          <div style={{ width: isMobile?'100%':320, background:'#fff', borderRight: isMobile?'none':'1px solid #f0f0f0', display:'flex', flexDirection:'column', overflowY:'auto', flexShrink:0 }}>
            ...
            <div style={{ padding:16, borderBottom:'1px solid #f0f0f0' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
                {[{label:'Total',value:vehicles.length,color:'#6d28d9'},{label:'Moving',value:moving,color:'#22c55e'},{label:'Stopped',value:stopped,color:'#ef4444'},{label:'Low Fuel',value:lowFuel,color:'#f59e0b'}].map((s,i) => (
              ...
              <div style={{ color:'#9ca3af', fontSize:10, marginTop:8, textAlign:'center' }}>
                Updated: {lastUpdate.toLocaleTimeString()} · Auto-refresh 10s
              </div>
            </div>
            ...
            <div style={{ padding:12, flex:1, overflowY:'auto' }}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                {vehicles.map(v => (
                  <div key={v.id}
                    onClick={() => { setSelected(selected===v.id?null:v.id); if(isMobile) setShowMap(true) }}
                    style={{ background:selected===v.id?'#f5f3ff':'#fff', border:`${selected===v.id?'1.5px solid #6d28d9':'1px solid #f0f0f0'}`, borderRadius:12, padding:12, cursor:'pointer', transition:'all 0.2s' }}>
                    ...
                    <div style={{ display:'flex', gap:12, marginBottom:8 }}>
                      <div><div style={{ color:'#9ca3af', fontSize:10 }}>Speed</div><div style={{ color:'#111', fontSize:13, fontWeight:700 }}>{Math.round(v.speed)} km/h</div></div>
                      <div><div style={{ color:'#9ca3af', fontSize:10 }}>ETA</div><div style={{ color:'#111', fontSize:13, fontWeight:700 }}>{v.eta}</div></div>
                    </div>

                    <div style={{ marginBottom: v.total_distance>0?6:0 }}>
                      <div style={{ color:'#9ca3af', fontSize:10, marginBottom:3 }}>Fuel</div>
                      <FuelBar pct={Math.round(v.fuel)} />
                    </div>

                    {v.total_distance > 0 && (
                      <div style={{ marginTop:6 }}>
                        <div style={{ color:'#9ca3af', fontSize:10, marginBottom:3 }}>Progress ({v.distance_covered}/{v.total_distance} km)</div>
        ...
        {(!isMobile || showMap) && (
          <div style={{ flex:1, position:'relative', minWidth:0 }}>
            <MapContainer center={[22,78]} zoom={5} style={{ height:'100%', width:'100%' }}>
              ...
              {vehicles.map(v => (
                <Marker key={v.id} position={[v.lat, v.lon]} icon={createVehicleIcon(v.type, v.status)}
                  eventHandlers={{ click:() => setSelected(v.id) }}>
                  <Popup>
                    <div style={{ minWidth:180, fontFamily:'sans-serif' }}>
                      <div style={{ fontWeight:700, fontSize:14, marginBottom:6 }}>{v.type==='truck'?'🚛':'🚐'} {v.name}</div>
                      ...
                      <div style={{ fontSize:12, marginBottom:4 }}>Speed: <strong>{Math.round(v.speed)} km/h</strong> · ETA: <strong>{v.eta}</strong></div>
                      <div style={{ fontSize:12 }}>Fuel: <strong style={{ color:v.fuel<25?'#ef4444':'#22c55e' }}>{Math.round(v.fuel)}%</strong>{v.fuel<25&&<span style={{ color:'#ef4444' }}> ⚠️ Low!</span>}</div>
                    </div>
                  </Popup>
                </Marker>
              ))}
            ...
            <button onClick={refresh}
              style={{ position:'absolute', top:16, left:16, zIndex:1000, background:'#fff', border:'1px solid #e5e7eb', borderRadius:10, padding:'8px 14px', color:'#374151', fontSize:12, cursor:'pointer', fontFamily:'inherit', fontWeight:500, boxShadow:'0 2px 8px rgba(0,0,0,0.08)' }}>
              ↻ Refresh
            </button>

            {/* Legend */}
            <div style={{ position:'absolute', top:16, right:16, zIndex:1000, background:'rgba(255,255,255,0.96)', border:'1px solid #e5e7eb', borderRadius:14, padding:'12px 14px', boxShadow:'0 2px 8px rgba(0,0,0,0.08)' }}>
              <div style={{ color:'#374151', fontSize:12, fontWeight:600, marginBottom:8 }}>Status</div>
            ...
            {lowFuel > 0 && (
              <div style={{ position:'absolute', bottom:24, left:'50%', transform:'translateX(-50%)', background:'#fef2f2', border:'1px solid #fecaca', borderRadius:12, padding:'10px 20px', color:'#b91c1c', fontSize:12, fontWeight:600, zIndex:1000, whiteSpace:'nowrap', boxShadow:'0 4px 16px rgba(239,68,68,0.15)' }}>
                ⚠️ {lowFuel} vehicle{lowFuel>1?'s':''} need refuelling!
              </div>
            )}
          </div>
        )}
      ...
      {showUpgrade && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:99999, padding:20 }}>
          <div style={{ background:'#fff', borderRadius:24, padding: isMobile?24:40, maxWidth:480, width:'100%', textAlign:'center', boxShadow:'0 25px 60px rgba(0,0,0,0.2)' }}>
            <div style={{ fontSize:48, marginBottom:16 }}>🚛</div>
            <h2 style={{ color:'#111', fontSize:22, fontWeight:800, marginBottom:8 }}>Upgrade to Business</h2>
            <p style={{ color:'#6b7280', fontSize:14, marginBottom:24, lineHeight:1.7 }}>
              Real-time GPS for your fleet, live updates, analytics and much more.
            </p>
            <div style={{ background:'#f9fafb', borderRadius:12, padding:16, marginBottom:20, textAlign:'left', border:'1px solid #f0f0f0' }}>
              {['Live GPS for 50 vehicles','Updates every 5 seconds','Driver analytics','Fuel reports','5 team accounts','Priority support'].map((f,i) => (
            ...
            <div style={{ display:'flex', gap:10 }}>
              ...
              <Link to="/pricing" style={{ flex:2, textDecoration:'none' }}>
                <button onClick={() => setShowUpgrade(false)}
                  style={{ width:'100%', padding:'12px', background:'#6d28d9', border:'none', borderRadius:12, color:'#fff', fontSize:14, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>
                  View Plans →
                </button>
              </Link>
            </div>
